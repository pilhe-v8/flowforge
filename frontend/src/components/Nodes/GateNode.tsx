...
export function GateNode({ data, selected, id }: NodeProps) {
  ...
  return (
    <div className={[
      'relative rounded-lg border-2 p-3 min-w-[180px] bg-blue-50 border-blue-300',
      ...
      <div className="flex items-center gap-2">
        ...
        <span className="font-medium text-sm">{d.label || 'Gate'}</span>
      </div>
      <div className="text-xs text-gray-500 mt-1">{d.rules.length} rule{d.rules.length !== 1 ? 's' : ''}</div>
